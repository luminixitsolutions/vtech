...
$sql = "SELECT id FROM tbl_rooftop_distibute_item_details WHERE code='' OR code IS NULL";
$row = getList($sql);
foreach($row as $result){
    ...
    $Code2 = $Code."".$id;
    $sql = "UPDATE tbl_rooftop_distibute_item_details SET code='$Code2' WHERE id='$id'";
    $conn->query($sql);
}
?>
<!DOCTYPE html>
       ...
       <!--  <th>Rate</th>
        ...
        <tbody id="dynamic_field" >
            <?php 
             ...
             $row12 = getList($sql12);
  foreach($row12 as $result){
    $sql11 = "SELECT SUM(Qty) AS CrQty FROM tbl_rooftop_distibute_item_details WHERE ProductId='".$result['ProductId']."' AND ProdType=0 AND BranchId='".$_REQUEST['BranchId']."'";
    $row11 = getRecord($sql11);
    $CrQty = $row11['CrQty'];

    $sql12 = "SELECT SUM(Qty) AS DrQty FROM tbl_rooftop_distibute_item_details2 WHERE ProductId='".$result['ProductId']."' AND ProdType=0 AND BranchId='".$_REQUEST['BranchId']."'";
...
<br>
<?php 
                                        $sql223 = "SELECT * FROM tbl_rooftop_distibute_item_details WHERE ProdType='2' AND SerialNo!='' AND BranchId='".$_REQUEST['BranchId']."'";
...
<div class="col-lg-12">                                      
 <table id="example" class="table table-striped table-bordered" width="100%">
  ...
  <?php $row223 = getList($sql223);
    ...
    $sql33 = "SELECT * FROM tbl_rooftop_distibute_item_details2 WHERE ProdType=2 AND SerialNo='".$result['SerialNo']."' AND BranchId='".$_REQUEST['BranchId']."'";
...
<div class="col-lg-12">                                      
 <table id="example3" class="table table-striped table-bordered" width="100%">
  ...
  <?php $row22 = getList($sql22);
    ...
    $sql33 = "SELECT * FROM tbl_rooftop_distibute_item_details2 WHERE ProdType=1 AND SerialNo='".$result['SerialNo']."' AND BranchId='".$_REQUEST['BranchId']."'";
    $rncnt33 = getRow($sql33);
    if($rncnt33 > 0){}
    else{?>
<tr>
