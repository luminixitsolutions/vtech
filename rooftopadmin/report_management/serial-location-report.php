...
include_once 'inc-rooftop-serial-location-report.php';
...
$canAccess = ($Roll === 1 || $Roll === 7 || $Roll === 26 || $Roll === 27
    || in_array('183', $Options) || in_array('101', $Options) || in_array('103', $Options)
...
if ($doSearch) {
    if ($filterSerial !== '') {
        ...
        if ($filterBranch > 0) {
            $brName = '';
            $brRow = getRecord("SELECT Name FROM tbl_rooftop_branch WHERE id='" . (int) $filterBranch . "' LIMIT 1");
...
if ($Roll === 1 || $Roll === 7) {
    ...
    $br = $conn->query("SELECT id, Name FROM tbl_rooftop_branch WHERE Status='1' AND id='" . (int) $sessionBranchId . "' ORDER BY Name");
}
if ($br) {
...
<div class="container-fluid flex-grow-1 container-p-y">
    <h4 class="font-weight-bold py-3 mb-0">Rooftop — Serial No — Location Report</h4>
