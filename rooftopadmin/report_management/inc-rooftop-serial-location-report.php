...
function serial_report_transfer_meta($conn)
{
    $hasTbl = false;
    $hasCol = false;
    $t1 = $conn->query("SHOW TABLES LIKE 'tbl_rooftop_dispatch_to_store_transfer_details'");
    if ($t1 && $t1->num_rows > 0) {
        $hasTbl = true;
        $c = $conn->query("SHOW COLUMNS FROM tbl_rooftop_dispatch_to_store_transfer_details LIKE 'Detail2Id'");
        if ($c && $c->num_rows > 0) {
            $hasCol = true;
        }
    }
...
{
    if ($hasTbl && $hasCol) {
        return "LEFT JOIN (SELECT DISTINCT Detail2Id FROM tbl_rooftop_dispatch_to_store_transfer_details WHERE Detail2Id IS NOT NULL) td_open ON td_open.Detail2Id = d2.id";
    }
    return '';
}
...
function serial_report_fetch_rows($conn, $branchId, $serialQ, $locationFilter)
{
    ...
    if ($meta['has_transfer_tbl'] && $meta['has_detail2_col']) {
        $reservedJoin = "LEFT JOIN (
            ...
            FROM tbl_rooftop_dispatch_to_store_transfer_details td
            INNER JOIN tbl_rooftop_dispatch_to_store_transfer t ON t.id = td.TransferId
            INNER JOIN tbl_rooftop_distibute_item_details2 d2r ON d2r.id = td.Detail2Id
            LEFT JOIN tbl_rooftop_branch tb ON tb.id = t.ToBranchId
    ...
    FROM (
        SELECT DISTINCT TRIM(SerialNo) AS serial_no FROM (
            SELECT SerialNo FROM tbl_rooftop_distibute_item_details d
            ...
            UNION
            SELECT SerialNo FROM tbl_rooftop_distibute_item_details2 d2
            ...
            UNION
            SELECT SerialNo FROM tbl_rooftop_stocks st
    ...
    LEFT JOIN (
        SELECT d2.* FROM tbl_rooftop_distibute_item_details2 d2
        $d2Join
        INNER JOIN (
            SELECT SerialNo, MAX(id) AS max_id FROM tbl_rooftop_distibute_item_details2
        ...
        AND NOT EXISTS (
            SELECT 1 FROM tbl_rooftop_stocks sx
            WHERE sx.CrDr='dr' AND sx.ProdType=1 AND sx.SerialNo = d2.SerialNo
        )
    ) off_ln ON off_ln.SerialNo = u.serial_no $serialJoinOff
    LEFT JOIN tbl_rooftop_distibute_items2 off_dist ON off_dist.id = off_ln.DistId
    LEFT JOIN tbl_users u_off ON u_off.id = off_ln.StoreExeId
    LEFT JOIN tbl_rooftop_branch b_off ON b_off.id = off_ln.BranchId
    LEFT JOIN (
        SELECT d.* FROM tbl_rooftop_distibute_item_details d
        INNER JOIN (
            SELECT SerialNo, MAX(id) AS max_id FROM tbl_rooftop_distibute_item_details
            WHERE ProdType IN (1,2) AND SerialNo IS NOT NULL AND TRIM(SerialNo) <> ''
            " . ($branchId > 0 ? " AND BranchId='" . $branchId . "' " : '') . "
            GROUP BY SerialNo
        ) sm ON sm.max_id = d.id
        WHERE NOT EXISTS (
            SELECT 1 FROM tbl_rooftop_distibute_item_details2 x
    ...
    LEFT JOIN tbl_rooftop_distibute_items st_dist ON st_dist.id = st_ln.DistId
    LEFT JOIN tbl_rooftop_branch b_st ON b_st.id = st_ln.BranchId
    LEFT JOIN (
        SELECT st.* FROM tbl_rooftop_stocks st
        INNER JOIN (
            SELECT SerialNo, MAX(id) AS max_id FROM tbl_rooftop_stocks
    ...
    LEFT JOIN tbl_rooftop_sell sell ON sell.id = cust.SellId
...
function serial_report_fetch_track_records($conn, $serialQ, $branchId = 0)
{
    ...
    $sqlStore = "SELECT d.CreatedDate AS sort_date, d.SerialNo, d.ProductName, d.ModelNo, d.Qty,
        ...
        FROM tbl_rooftop_distibute_item_details d
        LEFT JOIN tbl_rooftop_distibute_items h ON h.id = d.DistId
        LEFT JOIN tbl_rooftop_branch b ON b.id = d.BranchId
        WHERE d.ProdType IN (1,2) AND d.SerialNo IS NOT NULL AND TRIM(d.SerialNo) <> '' AND d.SerialNo <> 'N/A'
        $like $branchD";
    ...
    $sqlOfficer = "SELECT d2.CreatedDate AS sort_date, d2.SerialNo, d2.ProductName, d2.ModelNo, d2.Qty,
        ...
        FROM tbl_rooftop_distibute_item_details2 d2
        LEFT JOIN tbl_rooftop_distibute_items2 h2 ON h2.id = d2.DistId
        LEFT JOIN tbl_rooftop_branch b ON b.id = d2.BranchId
    ...
    $sqlChallan = "SELECT COALESCE(s.CreatedDate, CONCAT(s.InvoiceDate, ' 00:00:00'), sp.SellDate) AS sort_date,
        ...
        FROM tbl_rooftop_sell_products sp
        INNER JOIN tbl_rooftop_sell s ON s.id = sp.SellId AND s.Status = 1
        LEFT JOIN tbl_rooftop_branch b ON b.id = sp.BranchId
    ...
    if ($meta['has_transfer_tbl'] && $meta['has_detail2_col']) {
        $sqlRes = "SELECT COALESCE(t.CreatedDate, t.TransferDate) AS sort_date, d2.SerialNo, d2.ProductName, d2.ModelNo, td.Qty,
            ...
            FROM tbl_rooftop_dispatch_to_store_transfer_details td
            INNER JOIN tbl_rooftop_dispatch_to_store_transfer t ON t.id = td.TransferId
            INNER JOIN tbl_rooftop_distibute_item_details2 d2 ON d2.id = td.Detail2Id
            LEFT JOIN tbl_rooftop_branch tb_to ON tb_to.id = t.ToBranchId
    ...
    $sqlCust = "SELECT st.CreatedDate AS sort_date, st.SerialNo, st.ProductName, st.ModelNo, st.Qty,
        ...
        FROM tbl_rooftop_stocks st
        LEFT JOIN tbl_rooftop_branch b ON b.id = st.BranchId
        LEFT JOIN tbl_users uo ON uo.id = st.CreatedBy
        LEFT JOIN tbl_rooftop_sell sell ON sell.id = st.SellId
        LEFT JOIN tbl_users tu ON tu.id = sell.CustId
        WHERE st.ProdType = 1 AND st.CrDr = 'dr' AND st.SerialNo IS NOT NULL AND TRIM(st.SerialNo) <> '' AND st.SerialNo <> 'N/A'
        AND NOT EXISTS (
            SELECT 1 FROM tbl_rooftop_sell_products sp2
            INNER JOIN tbl_rooftop_sell s2 ON s2.id = sp2.SellId AND s2.Status = 1
...
function serial_report_fetch_rows_by_serial($conn, $serialQ, $branchId, $locationFilter)
{
    ...
    $sqlSn = "SELECT DISTINCT TRIM(SerialNo) AS serial_no FROM (
        SELECT SerialNo FROM tbl_rooftop_distibute_item_details
        WHERE ProdType IN (1,2) AND SerialNo LIKE '%$escQ%' $branchExtra
        UNION
        SELECT SerialNo FROM tbl_rooftop_distibute_item_details2
        WHERE ProdType IN (1,2) AND SerialNo LIKE '%$escQ%' $branchExtra
        UNION
        SELECT SerialNo FROM tbl_rooftop_stocks
