...
function contractorPaymentEnsureTable($conn)
{
    ...
    $sql = "CREATE TABLE IF NOT EXISTS tbl_rooftop_contractor_commission_payment (
...
function contractorPaymentEnsureAdvanceTable($conn)
{
    ...
    $sql = "CREATE TABLE IF NOT EXISTS tbl_rooftop_contractor_advance_payment (
      id INT UNSIGNED NOT NULL AUTO_INCREMENT,
      ContractorId INT NOT NULL,
      Amount DECIMAL(12,2) NOT NULL DEFAULT 0.00,
      PaymentDate DATE NOT NULL,
...
function contractorPaymentCommissionTotal($conn, $contractorId)
{
    ...
    $sql = "SELECT Amount FROM tbl_rooftop_made_contractor_commision WHERE ContractorId='$contractorId'";
    $res = $conn->query($sql);
    $total = 0.0;
    if ($res) {
...
function contractorPaymentPaidTotal($conn, $contractorId)
{
    ...
    $row = getRecord("SELECT COALESCE(SUM(Amount), 0) AS total FROM tbl_rooftop_contractor_commission_payment WHERE ContractorId='$contractorId' AND Status='1'");
...
function contractorPaymentAdvanceTotal($conn, $contractorId)
{
    ...
    $row = getRecord("SELECT COALESCE(SUM(Amount), 0) AS total FROM tbl_rooftop_contractor_advance_payment WHERE ContractorId='$contractorId' AND Status='1'");
    return contractorPaymentAmountValue($row['total'] ?? 0);
}
...
function contractorPaymentSave($conn, $contractorId, $amount, $paymentDate, $paymentMode, $referenceNo, $narration, $userId)
{
    ...
    $sql = "INSERT INTO tbl_rooftop_contractor_commission_payment SET
...
function contractorPaymentHistoryRows($conn, $contractorId)
{
    ...
    return getList("SELECT p.*, u.Fname AS CreatedByName
        FROM tbl_rooftop_contractor_commission_payment p
...
function contractorPaymentAdvanceSave($conn, $contractorId, $amount, $paymentDate, $paymentMode, $referenceNo, $narration, $userId)
{
    ...
    $sql = "INSERT INTO tbl_rooftop_contractor_advance_payment SET
        ContractorId='$contractorId',
        Amount='$amount',
        PaymentDate='$paymentDateEsc',
        PaymentMode='$paymentModeEsc',
...
function contractorPaymentAdvanceHistoryRows($conn, $contractorId)
{
    ...
    return getList("SELECT p.*, u.Fname AS CreatedByName
        FROM tbl_rooftop_contractor_advance_payment p
        LEFT JOIN tbl_users u ON u.id = p.CreatedBy
        WHERE p.ContractorId='$contractorId' AND p.Status='1'
        ORDER BY p.PaymentDate DESC, p.id DESC");
}
...
function contractorCommissionCustomerMetaMap($conn, array $customerIds)
{
    ...
    $sql = "SELECT tu.id, tu.District, tu.ProjectId, tu.ProjectSubHeadId,
        ...
        FROM tbl_users tu
        LEFT JOIN tbl_rooftop_common_master tcm ON tcm.id = tu.ProjectId
        LEFT JOIN tbl_rooftop_project_sub_head tps ON tps.id = tu.ProjectSubHeadId
        ...
        WHERE tu.id IN ($idList) AND tu.ProjectType = 2";
    $res = $conn->query($sql);
    $map = [];
    if ($res) {
        while ($row = $res->fetch_assoc()) {
...
function contractorBillingProjectsList($conn)
{
    return getList("SELECT cm.id, cm.Name,
            (SELECT COUNT(DISTINCT tc.CustId)
                FROM tbl_rooftop_made_contractor_commision tc
                INNER JOIN tbl_users cust ON cust.id = tc.CustId AND cust.ProjectType = 2
                WHERE cust.ProjectId = cm.id) AS site_count
        FROM tbl_rooftop_common_master cm
...
function contractorBillingSubHeadsList($conn, $projectId)
{
    ...
    return getList("SELECT tps.id, tps.Name,
        ...
        FROM tbl_rooftop_project_sub_head tps
...
function contractorBillingGetProjectName($conn, $projectId)
{
    ...
    $row = getRecord("SELECT Name FROM tbl_rooftop_common_master WHERE id = '$projectId' LIMIT 1");
...
function contractorBillingGetSubHeadName($conn, $subheadId)
{
    ...
    $row = getRecord("SELECT Name FROM tbl_rooftop_project_sub_head WHERE id = '$subheadId' LIMIT 1");
    return trim((string) ($row['Name'] ?? ''));
}
...
function contractorBillingProjectSubHeadSummaryList($conn, $projectId = 0)
{
    ...
    $sql = "SELECT proj.id AS project_id, proj.Name AS project_name,
        ...
        FROM tbl_rooftop_made_contractor_commision tc
        INNER JOIN tbl_users cust ON cust.id = tc.CustId AND cust.ProjectType = 2
        INNER JOIN tbl_rooftop_common_master proj ON proj.id = cust.ProjectId AND proj.Status = '1' AND proj.Roll = '24'
        INNER JOIN tbl_rooftop_project_sub_head psh ON psh.id = cust.ProjectSubHeadId
        INNER JOIN tbl_users tu ON tu.id = tc.ContractorId AND tu.Roll = '40'
        WHERE 1=1 $filterSql
