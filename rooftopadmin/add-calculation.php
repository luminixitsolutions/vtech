...
$foodCol = $conn->query("SHOW COLUMNS FROM tbl_rooftop_trip_details LIKE 'Food'");
if (!$foodCol || $foodCol->num_rows === 0) {
    $conn->query("ALTER TABLE tbl_rooftop_trip_details ADD COLUMN Food DECIMAL(12,2) NOT NULL DEFAULT 0.00");
}
...
$id = $_GET['id'];
$sql = "SELECT tpd.*,tu.VehAverage,tpd.VehAverage AS TpdVehAverage FROM tbl_rooftop_trip_details tpd INNER JOIN tbl_users tu ON tpd.DriverId=tu.id WHERE tpd.id='$id'";
$row7 = getRecord($sql);
...
$sql22 = "SELECT SUM(Amount) AS DieselPayment FROM tbl_rooftop_diesel_amount WHERE TripId='$id'";
...
<script type="text/javascript">
    var markers = [
        <?php 
        $sql = "SELECT StartLattitude,StartLongitude,EndLattitude,EndLongitude FROM tbl_rooftop_trip_details WHERE id='".$_GET['id']."'";
        $row = getRecord($sql);?>
        {
            "title": 'Start Trip',
