...
function serviceAbstractBuildWhere($filters, $exclude = array()) {
    $parts = array('tu.ProjectType = 2', 'tu.Roll = 5');
...
function serviceAbstractGetCountMap($filters, $groupColumn, $exclude = array()) {
    ...
    $sql = "SELECT
        ...
        FROM tbl_rooftop_service_complaint ts
        $where
        AND $groupColumn > 0
...
function serviceAbstractGetProjectHeads() {
    return getList("SELECT id, Name FROM tbl_rooftop_common_master WHERE Status = 1 AND Roll = 24 ORDER BY Name ASC");
}

function serviceAbstractGetSubHeads($projid) {
    $projid = (int) $projid;
    if ($projid <= 0) {
        return getList("SELECT id, Name, UnderBy FROM tbl_rooftop_project_sub_head WHERE Status = 1 ORDER BY Name ASC");
    }
    return getList("SELECT id, Name, UnderBy FROM tbl_rooftop_project_sub_head WHERE Status = 1 AND UnderBy = '$projid' ORDER BY Name ASC");
}
...
function serviceAbstractBuildTitle($filters) {
    ...
    if (!empty($filters['subheadid'])) {
        $row = getRecord("SELECT Name FROM tbl_rooftop_project_sub_head WHERE id = '" . (int) $filters['subheadid'] . "'");
        if (!empty($row['Name'])) {
            $parts[] = strtoupper($row['Name']);
        }
    } elseif (!empty($filters['projid'])) {
        $row = getRecord("SELECT Name FROM tbl_rooftop_common_master WHERE id = '" . (int) $filters['projid'] . "'");
        if (!empty($row['Name'])) {
            $parts[] = strtoupper($row['Name']);
        }
    }
...
function serviceAbstractBuildSubProjectHeadRows($filters) {
    ...
    if ($subheadid > 0) {
        $row = getRecord("SELECT id, Name FROM tbl_rooftop_project_sub_head WHERE id = '$subheadid' AND Status = 1");
...
function serviceAbstractListUrl($row, $filters, $filter = '') {
    ...
    return 'view-service-module.php?' . http_build_query($params);
}
