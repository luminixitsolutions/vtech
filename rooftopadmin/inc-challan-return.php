<?php
/**
 * Rooftop challan return / edit helpers.
 */
...
function challanReturnEnsureTables($conn)
{
    ...
    $conn->query("CREATE TABLE IF NOT EXISTS rooftop_challan_returns (
    ...
    $conn->query("CREATE TABLE IF NOT EXISTS rooftop_challan_return_items (
        id INT(11) NOT NULL AUTO_INCREMENT,
    ...
    $conn->query("CREATE TABLE IF NOT EXISTS rooftop_challan_edit_log (
        id INT(11) NOT NULL AUTO_INCREMENT,
        sell_id INT(11) NOT NULL DEFAULT 0,
        return_id INT(11) NOT NULL DEFAULT 0,
    ...
    $col = $conn->query("SHOW COLUMNS FROM tbl_rooftop_sell LIKE 'ReturnStatus'");
    if ($col && $col->num_rows === 0) {
        $conn->query("ALTER TABLE tbl_rooftop_sell ADD COLUMN ReturnStatus TINYINT(1) NOT NULL DEFAULT 0 AFTER Status");
    }
}
...
function challanReturnGetSellItems($conn, $sellId)
{
    ...
    $rows = getList("SELECT * FROM tbl_rooftop_sell_products WHERE SellId='$sellId' AND ProductName!='' ORDER BY id ASC");
...
function challanReturnAlreadyReturned($conn, $sellId)
{
    ...
    $sellId = (int) $sellId;
    return getRow("SELECT id FROM rooftop_challan_returns WHERE sell_id='$sellId' LIMIT 1") > 0;
}
...
function challanReturnLogActivity($conn, $sellId, $returnId, $actionType, $oldItems, $newItems, $remarks, $performedBy)
{
    ...
    $sql = "INSERT INTO rooftop_challan_edit_log SET
        sell_id='$sellId',
        return_id='$returnId',
...
function challanReturnRestoreStock($conn, $sellId, array $items)
{
    ...
    foreach ($items as $item) {
        if (challanReturnIsSerialItem($item)) {
            ...
            if ($distId > 0) {
                $conn->query("UPDATE tbl_rooftop_distibute_item_details2 SET SellStatus=0, SellId=0 WHERE id='$distId'");
            }
        }
    }
    $conn->query("DELETE FROM tbl_rooftop_stocks WHERE SellId='$sellId' AND SellType='Challan'");
}
...
function challanReturnDeductStock($conn, $sellId, $branchId, $invoiceDate, $narration, $userId, array $bulkItems, array $serialItems)
{
    ...
    foreach ($bulkItems as $item) {
        ...
        $sql22 = "INSERT INTO tbl_rooftop_sell_products SET UserId='$custId',SellId='$sellId',ProductName='$productName',Purity='$purity',Qty='$qty',ProductId='$productId',ModelNo='$modelNo',SellDate='$invoiceDate',SerialNo='$serialNo',BranchId='$branchId'";
        if (!$conn->query($sql22)) {
            return ['success' => false, 'message' => 'Failed to save challan product.'];
        }
        $postId = (int) mysqli_insert_id($conn);

        $sqlStock = "INSERT INTO tbl_rooftop_stocks SET SellId='$sellId',ProductId='$productId',ProductName='$productName',Qty='$qty',Status='1',CrDr='dr',CreatedBy='$userId',CreatedDate='$invoiceDate',Narration='$narration',PostId='$postId',BranchId='$branchId',SellType='Challan',SerialNo='$serialNo',ModelNo='$modelNo',ProdType='0'";
    ...
    foreach ($serialItems as $item) {
        ...
        $dist = getRecord("SELECT * FROM tbl_rooftop_distibute_item_details2 WHERE id='$distId' LIMIT 1");
        if (!$dist) {
        ...
        $sql22 = "INSERT INTO tbl_rooftop_sell_products SET UserId='$custId',SellId='$sellId',ProductName='$productName',Purity='$purity',Qty='1',ProductId='$distId',ModelNo='$modelNo',SellDate='$invoiceDate',SerialNo='$serialNo',BranchId='$branchId'";
        if (!$conn->query($sql22)) {
            return ['success' => false, 'message' => 'Failed to save serial challan product.'];
        }
        $postId = (int) mysqli_insert_id($conn);

        $sqlStock = "INSERT INTO tbl_rooftop_stocks SET SellId='$sellId',ProductId='$distId',ProductName='$productName',Qty='1',Status='1',CrDr='dr',CreatedBy='$userId',CreatedDate='$invoiceDate',Narration='$narration',PostId='$postId',BranchId='$branchId',SellType='Challan',SerialNo='$serialNo',ModelNo='$modelNo',ProdType='1'";
        if (!$conn->query($sqlStock)) {
            return ['success' => false, 'message' => 'Failed to update stock for serial item.'];
        }

        if (!$conn->query("UPDATE tbl_rooftop_distibute_item_details2 SET SellId='$sellId',SellStatus=1 WHERE id='$distId'")) {
...
function challanReturnProcess($conn, $sellId, $returnDate, $remarks, $userId)
{
    ...
    try {
        ...
        $now = date('Y-m-d H:i:s');
        $sqlReturn = "INSERT INTO rooftop_challan_returns SET sell_id='$sellId', return_date='$returnDate', remarks='$remarksEsc', created_by='$userId', created_at='$now', updated_at='$now'";
        ...
        foreach ($items as $item) {
            ...
            $sqlItem = "INSERT INTO rooftop_challan_return_items SET return_id='$returnId', sell_id='$sellId', product_id='$productId', item_id='$itemId', serial_no='$serialNo', qty='$qty', prod_type='$prodType', product_name='$productName', model_no='$modelNo', unit='$unit', created_at='$now'";
        ...
        if (!$conn->query("UPDATE tbl_rooftop_sell SET ReturnStatus=1, ContractorAssignStatus=0, ContractorAssignId=0 WHERE id='$sellId'")) {
...
function challanReturnEditProcess($conn, $sellId, $remarks, $userId, array $bulkItems, array $serialDistIds, $custId, $branchId, $invoiceDate, $narration)
{
    ...
    $returnRec = getRecord("SELECT * FROM rooftop_challan_returns WHERE sell_id='$sellId' LIMIT 1");
    if (!$returnRec) {
    ...
    foreach ($preparedBulk as $item) {
        $productId = (int) ($item['ProductId'] ?? 0);
        $qty = (float) ($item['Qty'] ?? 0);
        $sql11 = "SELECT SUM(Qty) AS CrQty FROM tbl_rooftop_distibute_item_details2 WHERE ProductId='$productId'";
        $row11 = getRecord($sql11);
        $crQty = (float) ($row11['CrQty'] ?? 0);
        $sqlDr = "SELECT COALESCE(SUM(Qty), 0) AS DrQty FROM tbl_rooftop_stocks WHERE CrDr='dr' AND ProductId='$productId'";
    ...
    $conn->begin_transaction();
    try {
        if (!$conn->query("DELETE FROM tbl_rooftop_sell_products WHERE SellId='$sellId'")) {
        ...
        if (!$conn->query("UPDATE tbl_rooftop_sell SET ReturnStatus=0, ContractorAssignStatus=0, ContractorAssignId=0, ContractorAssignDate=NULL WHERE id='$sellId'")) {
