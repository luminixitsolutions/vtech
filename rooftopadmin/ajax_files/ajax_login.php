...
$_SESSION['rooftop_login_otp_hash'] = password_hash($otp, PASSWORD_DEFAULT);
$_SESSION['rooftop_login_otp_expires'] = time() + 600;
$_SESSION['rooftop_login_pending_id'] = (int) $row['id'];
$_SESSION['rooftop_login_otp_attempts'] = 0;
if (!empty($RooftopLoginOtpDevPrefill)) {
    $_SESSION['rooftop_login_otp_prefill'] = $otp;
} else {
    unset($_SESSION['rooftop_login_otp_prefill']);
}
...
$masked = strlen($phone) >= 4
    ...
    : '****';
$_SESSION['rooftop_login_phone_mask'] = $masked;

$smsEnabled = !empty($RooftopLoginOtpSendSms);
...
if (!$shouldSendSms) {
    error_log('Rooftop admin login OTP (user id ' . (int) $row['id'] . '): ' . $otp);
    exit;
}
