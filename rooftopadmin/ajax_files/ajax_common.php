...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_rooftop_common_master WHERE id = '$id'";
    $result = $conn->query($query);
...
if ($result && mysqli_num_rows($result) > 0) {
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_rooftop_common_master WHERE id = '$id'";
      $conn->query($query);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_rooftop_common_master SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
 ...
 $srno = 1;
  ...
  $sql = "SELECT * FROM tbl_rooftop_common_master WHERE Roll='$Roll' ORDER BY id DESC";
   $rx = $conn->query($sql);
  while($nx = $rx->fetch_assoc()){
        $sql2 = "SELECT Name As PumpHead FROM tbl_rooftop_common_master 
