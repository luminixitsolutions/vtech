...
FROM tbl_rooftop_dispatch_to_store_transfer t 
LEFT JOIN tbl_rooftop_branch tb ON t.ToBranchId=tb.id 
...
WHERE $where ORDER BY t.id DESC";
                                    ...
                                    while ($row = $res->fetch_assoc()) {
                                        $tid = $row['id'];
                                        $cnt = getRecord("SELECT COUNT(*) AS c FROM tbl_rooftop_dispatch_to_store_transfer_details WHERE TransferId='$tid'");
                                        $tot = getRecord("SELECT COALESCE(SUM(Qty),0) AS t FROM tbl_rooftop_dispatch_to_store_transfer_details WHERE TransferId='$tid'");
                                        ?>
                                        <tr>
                                            <td><?php echo $i; ?></td>
