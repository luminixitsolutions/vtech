...
$chkTd = $conn->query("SHOW COLUMNS FROM tbl_rooftop_dispatch_to_store_transfer_details LIKE 'Detail2Id'");
if ($chkTd && $chkTd->num_rows > 0) {
    $hasDetail2IdTd = true;
}
// Lines still linked in tbl_rooftop_dispatch_to_store_transfer_details must be hidden from dispatch qty.
        ...
        <div class="layout-container">
            ...
            <div class="layout-content">
                <div class="container-fluid flex-grow-1 container-p-y">
                    <h4 class="font-weight-bold py-3 mb-0">Rooftop — Dispatch Officer: Transfer Items to Store</h4>
                    <p class="text-muted">Transfer rooftop stock (assigned by admin) to a store.</p>
                    <div class="card mb-4">
                        <div class="card-body">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control" name="ToBranchId" id="ToBranchId" required onchange="this.form.submit()">
                                            ...
                                            <?php
                                            $sqlb = "SELECT * FROM tbl_rooftop_branch WHERE Status='1' ORDER BY Name";
                                            if ($Roll != 1 && $Roll != 7) {
                                                if (!empty($BranchId)) {
                                                    $sqlb = "SELECT * FROM tbl_rooftop_branch WHERE Status='1' AND id!='" . (int)$BranchId . "' ORDER BY Name";
                                                }
                                            }
                                            $rb = $conn->query($sqlb);
                            ...
                            if ($ToBranchId > 0) {
                                ...
                                <form method="post" action="item_transfer_workflow/save-dispatch-to-store-transfer.php">
                                    ...
                                    <table class="table table-bordered table-sm" id="tblQty">
                                        ...
                                        <tbody>
                                        <?php
                                        $sq = "SELECT d2.ProductId, d2.ProductName, d2.Purity, SUM(d2.Qty) AS AvailQty FROM tbl_rooftop_distibute_item_details2 d2 $d2JoinOpenTransfer WHERE d2.StoreExeId='$user_id' AND d2.ProdType=0 $d2WhereNotOpenTransfer GROUP BY d2.ProductId, d2.ProductName, d2.Purity HAVING AvailQty > 0";
                                        $rq = $conn->query($sq);
                                        $has_qty = false;
                                    ...
                                    <table class="table table-bordered table-sm" id="tblSerial">
                                        ...
                                        <tbody>
                                        <?php
                                        $ss = "SELECT d2.id, d2.ProductName, d2.SerialNo FROM tbl_rooftop_distibute_item_details2 d2 $d2JoinOpenTransfer WHERE d2.StoreExeId='$user_id' AND d2.ProdType IN (1,2) AND d2.SerialNo!='' $d2WhereNotOpenTransfer ORDER BY d2.ProductName, d2.SerialNo";
                                        $rs = $conn->query($ss);
                                        $has_serial = false;
                                    ...
                                    <div class="form-group">
                                        ...
                                        <a href="item_transfer_workflow/view-dispatch-to-store-transfers.php" class="btn btn-secondary">View My Transfers</a>
                                    </div>
                                </form>
                                <?php
                            } else {
