...
function delete_matching_store_row($conn, $ToBranchId, $transferId, $detail, $hasDetail2Id) {
    ...
    $tryDeleteById = function ($id) use ($conn) {
        ...
        if (!$conn->query("DELETE FROM tbl_rooftop_distibute_item_details WHERE id='$id' LIMIT 1")) {
            return false;
        }
        ...
        $chk = getRecord("SELECT id FROM tbl_rooftop_distibute_item_details WHERE id='$id'");
    ...
    if ($Detail2Id > 0 && $lenSuffix > 0) {
        ...
        $q = "SELECT id FROM tbl_rooftop_distibute_item_details WHERE BranchId='$escBranch' AND ProductId='$ProductId' AND ProdType='$ProdType' $serialClause
        ...
        if (empty($pickIds)) {
            $q2 = "SELECT id, code FROM tbl_rooftop_distibute_item_details WHERE BranchId='$escBranch' AND ProductId='$ProductId' AND ProdType='$ProdType' $serialClause";
            $res2 = $conn->query($q2);
            if ($res2) {
    ...
    $q = "SELECT id FROM tbl_rooftop_distibute_item_details WHERE $baseWhere";
    if ((string)$ModelNo !== '') {
    ...
    $q2 = "SELECT id FROM tbl_rooftop_distibute_item_details WHERE $baseWhere ORDER BY id DESC LIMIT 1";
    $row2 = getRecord($q2);
    if ($row2 && !empty($row2['id'])) {
        return $tryDeleteById((int)$row2['id']) ? true : false;
    }

    $cntRow = getRecord("SELECT COUNT(*) AS c FROM tbl_rooftop_distibute_item_details WHERE BranchId='$escBranch' AND ProductId='$ProductId' AND ProdType='$ProdType' AND ABS(Qty-$Qty)<0.01" . $serialClause);
...
$sql .= " FROM tbl_rooftop_dispatch_to_store_transfer_details d
INNER JOIN tbl_rooftop_dispatch_to_store_transfer t ON d.TransferId = t.id
...
foreach ($lines as $detail) {
    ...
    $ok = $ok && $conn->query("DELETE FROM tbl_rooftop_dispatch_to_store_transfer_details WHERE id='$detailPk' LIMIT 1");
}

if ($ok) {
    foreach (array_keys($transferIdsTouched) as $tid) {
        $tid = (int)$tid;
        $c = getRecord("SELECT COUNT(*) AS c FROM tbl_rooftop_dispatch_to_store_transfer_details WHERE TransferId='$tid'");
        if ($c && (int)$c['c'] === 0) {
            $conn->query("DELETE FROM tbl_rooftop_dispatch_to_store_transfer WHERE id='$tid' LIMIT 1");
        }
    }
    $conn->commit();
