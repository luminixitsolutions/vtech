                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover" id="example">
                                    ...
                                    <tbody>
                                    <?php
                                    $sql = "SELECT t.*, tb1.Name AS FromStoreName, tb2.Name AS ToStoreName FROM tbl_rooftop_store_to_store_transfer t LEFT JOIN tbl_rooftop_branch tb1 ON t.FromBranchId=tb1.id LEFT JOIN tbl_rooftop_branch tb2 ON t.ToBranchId=tb2.id WHERE $where ORDER BY t.id DESC";
                                    $res = $conn->query($sql);
                                    $i = 1;
                                    while ($row = $res->fetch_assoc()) {
                                        $tid = $row['id'];
                                        $cnt = getRecord("SELECT COUNT(*) AS c FROM tbl_rooftop_store_to_store_transfer_details WHERE TransferId='$tid'");
                                        $tot = getRecord("SELECT COALESCE(SUM(Qty),0) AS t FROM tbl_rooftop_store_to_store_transfer_details WHERE TransferId='$tid'");
                                        ?>
                                        <tr>
                                            <td><?php echo $i; ?></td>
