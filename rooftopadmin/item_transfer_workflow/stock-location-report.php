...
$d2JoinOpen = ($hasDispatchTransferTbl && $hasDetail2IdCol)
    ? "LEFT JOIN (SELECT DISTINCT Detail2Id FROM tbl_rooftop_dispatch_to_store_transfer_details WHERE Detail2Id IS NOT NULL) td_open ON td_open.Detail2Id = d2.id"
    : "";
$d2WhereOpen = ($hasDispatchTransferTbl && $hasDetail2IdCol) ? "AND td_open.Detail2Id IS NULL" : "";

/* ---- Store: net qty (Cr in tbl_rooftop_distibute_item_details minus Dr in tbl_rooftop_distibute_item_details2) ---- */
$rowsStoreQty = array();
if ($view === 'all' || $view === 'qty_store') {
    $sql = "SELECT b.id AS branch_id, b.Name AS branch_name, 'Store (net balance)' AS account_type,
        ...
        (COALESCE(SUM(d.Qty),0) - COALESCE((SELECT SUM(x.Qty) FROM tbl_rooftop_distibute_item_details2 x
    ...
    INNER JOIN tbl_rooftop_branch b ON b.id = d.BranchId
...
/* ---- Serial-level at store: in tbl_rooftop_distibute_item_details, not allocated in details2 ---- */
...
if ($view === 'all' || $view === 'serial') {
    ...
    FROM tbl_rooftop_distibute_item_details d
    INNER JOIN tbl_rooftop_branch br ON br.id = d.BranchId
    LEFT JOIN tbl_products tp ON tp.id = d.ProductId
...
$branches = array();
$br = $conn->query("SELECT id, Name FROM tbl_rooftop_branch WHERE Status='1' ORDER BY Name");
if ($br) {
        ...
        <div class="layout-container">
            ...
            <div class="layout-content">
                <div class="container-fluid flex-grow-1 container-p-y">
                    ...
                    <div class="alert alert-warning">Reserved section needs table <code>tbl_rooftop_dispatch_to_store_transfer_details</code> with column <code>Detail2Id</code>.</div>
                    <?php } ?>
