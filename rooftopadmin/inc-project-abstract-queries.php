...
function projectAbstractListSql($conn, $roll, $projectId, $subheadId, $dist = '', $val2 = '')
{
    ...
    if ($roll === 'capacity') {
        return "SELECT tu.* FROM tbl_users tu WHERE (tu.PumpCapacity='$val2Esc' OR tu.RooftopPlantCapacity='$val2Esc') $scope";
    }
    ...
    if ($roll === 'deliverychallan') {
        return "SELECT tu.* FROM tbl_users tu WHERE 1=1 $scope
            AND EXISTS (SELECT 1 FROM tbl_rooftop_sell ts WHERE ts.CustId=tu.id AND ts.SellType='Challan')";
    }

    if ($roll === 'dispatch') {
        return "SELECT tu.* FROM tbl_users tu WHERE 1=1 $scope
            AND EXISTS (SELECT 1 FROM tbl_rooftop_sell ts WHERE ts.CustId=tu.id)";
    }

    if ($roll === 'dispatchpending') {
        return "SELECT tu.* FROM tbl_users tu
            LEFT JOIN tbl_rooftop_sell ts ON tu.id=ts.CustId AND ts.Inst_Dispatcher_Otp_Verify=1
    ...
    if ($roll === 'installationpending') {
        return "SELECT tu.* FROM tbl_users tu
            ...
            AND (
                (
                    EXISTS (SELECT 1 FROM tbl_rooftop_sell ts WHERE ts.CustId=tu.id)
                    AND NOT EXISTS (
                        SELECT 1 FROM tbl_installations ti
                        WHERE ti.CustId=tu.id AND ti.InstallStatus='Yes'
                    )
