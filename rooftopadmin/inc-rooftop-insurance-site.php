...
function insuranceEnsureHistoryTable()
{
    ...
    $sql = "CREATE TABLE IF NOT EXISTS tbl_rooftop_insurance_site_history (
...
{
    return 'tdo.Inst_Dispatcher_Otp_Verify = 1 AND tu.Roll = 5 AND tu.ProjectType = 2';
}
...
{
    return 'FROM tbl_rooftop_sell tdo
...
function insuranceSiteRenewedSqlCondition()
{
    ...
    return insuranceSiteCompletedSqlCondition() . "
      AND EXISTS (
          SELECT 1 FROM tbl_rooftop_insurance_site_history h
...
{
    return "LEFT JOIN tbl_rooftop_installations ti ON ti.id = (
        SELECT MAX(ti2.id) FROM tbl_rooftop_installations ti2
...
function insuranceSaveSiteHistory($customer, $data, $userId, $meta = array())
{
    ...
    $conn->query("INSERT INTO tbl_rooftop_insurance_site_history SET
        CustId = '$custId',
...
function insuranceGetSiteHistoryByCustomer($custId)
{
    ...
    return getList("SELECT h.*, u.Fname AS ProcessorFname, u.Lname AS ProcessorLname
        FROM tbl_rooftop_insurance_site_history h
...
function insuranceMarkCustomerCompleted($custId, $data, $userId, $customer = array(), $meta = array())
{
    ...
    $inst = getRecord("SELECT id FROM tbl_rooftop_installations WHERE CustId = '$custId' AND Type = 2 ORDER BY id DESC LIMIT 1");
    if (!empty($inst['id'])) {
        $instId = (int) $inst['id'];
        $conn->query("UPDATE tbl_rooftop_installations SET
