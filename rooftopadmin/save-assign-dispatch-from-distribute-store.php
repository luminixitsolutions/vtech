...
$row77 = getRecord("SELECT Roll, Options, BranchId, RooftopBranchId FROM tbl_users WHERE id='$user_id'");
...
foreach ($distIds as $did) {
    $h = getRecord("SELECT * FROM tbl_rooftop_distibute_items WHERE id='$did' AND Status=1 LIMIT 1");
...
foreach ($distIds as $did) {
    $didInt = (int) $did;
    $alreadyAssigned = getRooftopStoreDistDispatchAssignment($conn, $didInt);
...
try {
    $sqlIns = "INSERT INTO tbl_rooftop_distibute_items2 SET BranchId='$BranchId',StoreInchId='0',StoreExeId='$StoreExeId',CreatedDate='$CreatedDateEsc',Narration='$NarrationEsc',Status='1',VehicalDate=$VehicalDateSql,VehicalNo=$VehicalNoSql";
    ...
    $q = "SELECT * FROM tbl_rooftop_distibute_item_details WHERE DistId IN ($idList)";
    $res = $conn->query($q);
    if (!$res) {
        throw new Exception($conn->error);
    }
    ...
    while ($d = $res->fetch_assoc()) {
        ...
        $sql22 = "INSERT INTO tbl_rooftop_distibute_item_details2 SET BranchId='$BranchId',StoreExeId='$StoreExeId',DistId='$newDist2Id',StoreInchId='0',
    ...
    foreach ($distIds as $didLog) {
        ...
        logRooftopStoreDistDispatchAction($conn, (int) $didLog, $newDist2Id, 'assign', $StoreExeId, (int) $user_id, $remarks);
        logRooftopPoDispatchActivityFromStoreDist($conn, (int) $didLog, 'dispatch_assign', $StoreExeId, (int) $user_id, (int) $inserted, $remarks);
    }
