<?php
/**
 * Detect whether a rooftop store distribute batch was handed to dispatch.
 */

function rooftopStoreDistDispatchEnsureLogTable($conn)
{
    ...
    $checked = true;
    $sql = "CREATE TABLE IF NOT EXISTS tbl_rooftop_store_dist_dispatch_log (
...
function logRooftopStoreDistDispatchAction($conn, $storeDistId, $dispatchDist2Id, $actionType, $dispatchOfficerId, $performedBy, $remarks = '')
{
    rooftopStoreDistDispatchEnsureLogTable($conn);
    ...
    $sql = "INSERT INTO tbl_rooftop_store_dist_dispatch_log SET
...
function revertRooftopStoreDistDispatchAssignment($conn, $storeDistId, $performedBy)
{
    $storeDistId = (int) $storeDistId;
    $performedBy = (int) $performedBy;
    if ($storeDistId < 1) {
    ...
    $assignment = getRooftopStoreDistDispatchAssignment($conn, $storeDistId);
    if ($assignment === null) {
    ...
    try {
        if (!$conn->query("DELETE FROM tbl_rooftop_distibute_item_details2 WHERE DistId='$h2Id'")) {
            throw new Exception($conn->error);
        }
        if (!$conn->query("DELETE FROM tbl_rooftop_distibute_items2 WHERE id='$h2Id'")) {
            throw new Exception($conn->error);
        }
        ...
        if (!logRooftopStoreDistDispatchAction($conn, $storeDistId, $h2Id, 'revert', $storeExeId, $performedBy, $remarks)) {
...
function parseRooftopStoreDistIdsFromDispatchNarration($narration)
{
...
function buildRooftopStoreDistDispatchOfficerMap($conn, array $storeDistIds = [])
{
    $map = [];
    $idFilter = [];
    ...
    $sqlNarr = "SELECT h2.Narration, u2.Fname AS OfficerName
        FROM tbl_rooftop_distibute_items2 h2
    ...
    if ($resNarr) {
        while ($row = $resNarr->fetch_assoc()) {
            ...
            foreach (parseRooftopStoreDistIdsFromDispatchNarration($row['Narration'] ?? '') as $storeDistId) {
...
function buildRooftopStoreDistTotQtyMap($conn, array $storeDistIds = [])
{
    $map = [];
    $ids = [];
    foreach ($storeDistIds as $id) {
    ...
    $idList = implode(',', $ids);
    $res = $conn->query("SELECT DistId, SUM(Qty) AS TotQty FROM tbl_rooftop_distibute_item_details WHERE DistId IN ($idList) GROUP BY DistId");
    if ($res) {
...
function getRooftopStoreDistDispatchAssignment($conn, $storeDistId)
{
    $id = (int) $storeDistId;
    if ($id < 1) {
        return null;
...
function isRooftopStoreDistAssignedToDispatch($conn, $storeDistId)
{
    return getRooftopStoreDistDispatchAssignment($conn, $storeDistId) !== null;
}
