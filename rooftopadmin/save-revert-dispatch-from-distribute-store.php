...
$h = getRecord("SELECT id FROM tbl_rooftop_distibute_items WHERE id='$storeDistId' AND Status=1 LIMIT 1");
...
$result = revertRooftopStoreDistDispatchAssignment($conn, $storeDistId, (int) $user_id);
if (!empty($result['ok'])) {
    if (is_array($assignment)) {
        ...
        logRooftopPoDispatchActivityFromStoreDist(
            $conn,
            $storeDistId,
            'dispatch_revert',
