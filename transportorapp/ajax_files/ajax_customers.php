...
$query = "SELECT * FROM tbl_users WHERE (Phone = '$Username' OR EmailId='$Username') AND Status=1 AND Roll IN(46)";
 $rncnt = getRow($query);
 $row = getRecord($query);
 if($rncnt > 0){
...
if($_POST['action']=='checkRunningTrip'){
    $uid = (int) $_POST['uid'];
    if ((int) ($_SESSION['User']['Roll'] ?? 0) === 46) {
        require_once dirname(__DIR__) . '/inc-transportor.php';
        if (!transportorGetDriver((int) $_SESSION['User']['id'], $uid)) {
            echo 1;
            exit;
        }
    }
    $sql = "SELECT * FROM tbl_trip_details WHERE DriverId='$uid' AND Status=0 LIMIT 1";
    $rncnt = getRow($sql);
    if($rncnt > 0){
        echo 1; //running
    }
