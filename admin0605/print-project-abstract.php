...
<?php 
    function getDetails($val,$dist,$val2){
        ...
        if($val == 'deliverychallan'){
            // Count delivery challans created (tbl_sell with SellType='Challan')
            $sql2 = "SELECT * FROM tbl_sell ts INNER JOIN tbl_users tu ON tu.id=ts.CustId WHERE ts.SellType='Challan' AND tu.ProjectType=1 AND tu.District='$dist' AND tu.ProjectId='".$_GET['projid']."' AND tu.ProjectSubHeadId='".$_GET['SubHeadProjectId']."'";
            $rncnt2 = getRow($sql2);
        }
        if($val == 'dispatch'){
