...
$user_id = $_SESSION['Admin']['id']; // Manager login
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'request_extension') {
        ...
        mysqli_query($conn,"
            ...
            (flow_id, requested_by, extension_days, status, requested_date)
            VALUES
            ('$flowId','$user_id',3,'PENDING',NOW())
        ");
        ...
        mysqli_query($conn,"
            ...
            (flow_id, action_by, action_type, remarks, action_date)
            VALUES
            ('$flowId','$user_id','EXTENSION_REQUEST','Extension requested for 3 days',NOW())
        ");
...
while ($row = $res->fetch_assoc()) {
    ...
    if ($pending >= 3) {
        $status = "<span class='badge badge-danger'>Escalation Required</span>";
    } else {
        $status = "<span class='badge badge-warning'>Within SLA</span>";
    }
...
<div class="modal-header">
    <h5 class="modal-title">Manager Follow-up</h5>
...
function requestExtension(flowId){
    if(!confirm('Request 3 days extension?')) return;
    ...
    $.post('',{
        ...
        if(res === 'ALREADY'){
            ...
            alert('Extension request sent to GM');
            location.reload();
        }
    });
}
