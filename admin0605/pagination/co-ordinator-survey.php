...
$sql = "SELECT tu.*,tut.Name As User_Type FROM tbl_users tu 
                    LEFT JOIN tbl_user_type tut ON tu.UserType=tut.id WHERE tu.Roll=5 AND tu.LeadCust=0 AND tu.ProjectType='1'";
 		}
else{
    $sql = "SELECT tu.*,tut.Name As User_Type FROM tbl_users tu LEFT JOIN tbl_user_type tut ON tu.UserType=tut.id WHERE tu.Roll=5 AND tu.CoordinatorId='$user_id' AND tu.ProjectType='1'"; 
}
...
$sql88 = "SELECT Fname FROM tbl_users WHERE id='".$row['TelSurveyBy']."'";
$row88 = getRecord($sql88);
    ...
    $data[] = array(
            ...
            "SurveyBy"=>$row88['Fname'],
            "BeneficiaryId"=>$row['BeneficiaryId'],
    		"Fname"=>'<a href="customer-profile.php?id='.$row['id'].'" target="_new">'.$row['Fname'] . ' ' . $row['Lname'].'</a>',
            "Phone"=>$row['Phone'],
            "Taluka"=>$row['Taluka'],
            "Village"=>$row['Village'],
