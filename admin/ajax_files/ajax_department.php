...
$qx = "INSERT INTO tbl_department SET Name = '$Name',Status='$Status',Photo='$Photo',Phone = '$Phone',Phone2='$Phone2',EmailId='$EmailId',Address='$Address'";
	$conn->query($qx);
	echo 1;
}
}
...
$query = "SELECT * FROM tbl_department WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_department SET Name = '$Name', Status='$Status',Photo='$Photo',Phone = '$Phone',Phone2='$Phone2',EmailId='$EmailId',Address='$Address' WHERE id = '$id'";
 	$conn->query($query2);
 }
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
       $query2 = "SELECT Photo FROM tbl_department WHERE id = '$id'";
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_department SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_department ORDER BY id DESC";
   $rx = $conn->query($sql);
