...
$qx = "INSERT INTO tbl_category SET Name = '$Name',Status='$Status',Photo='$Photo'";
	$conn->query($qx);
	echo 1;
}
}
...
$query = "SELECT * FROM tbl_category WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_category SET Name = '$Name', Status='$Status',Photo='$Photo' WHERE id = '$id'";
 	$conn->query($query2);
 }
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
       $query2 = "SELECT Photo FROM tbl_category WHERE id = '$id'";
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_category SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
