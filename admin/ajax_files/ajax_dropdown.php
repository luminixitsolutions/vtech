...
<span class="custom-control-label"><?php echo $rw['Name'];?></span>
    ...
    if($_POST['action'] == 'getDispatchOfficersByBranch'){
        $bid = (int) $_POST['id'];
        $sql77 = "SELECT Roll, BranchId FROM tbl_users WHERE id='$user_id' LIMIT 1";
        $row77 = getRecord($sql77);
        $Roll = (int) ($row77['Roll'] ?? 0);
        $userBranchId = (int) ($row77['BranchId'] ?? 0);
        if ($Roll != 1 && $Roll != 7 && $bid !== $userBranchId) {
            echo '<option value="" selected disabled>Unauthorized</option>';
        } elseif ($bid < 1) {
            echo '<option value="" selected disabled>Select store first</option>';
        } else {
            ?>
        <option value="" selected disabled>Select Dispatch Officier</option>
    <?php
            $q = "SELECT id, Fname FROM tbl_users WHERE Status='1' AND Roll=26
            ...
            if ($r) {
                while ($rw = $r->fetch_assoc()) {
                    ?>
                    <option value="<?php echo (int) $rw['id']; ?>"><?php echo htmlspecialchars($rw['Fname']); ?></option>
                    <?php
                }
            }
        }
