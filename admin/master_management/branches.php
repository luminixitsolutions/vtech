...
<h4 class="font-weight-bold py-3 mb-0">List Of Store
...
<div class="form-row">
<div class="form-group col">
<label class="form-label">Another Contact No </label>
<input type="text" name="Phone2" class="form-control" id="Phone2" placeholder="">
<div class="clearfix"></div>
</div>
</div>
...
<input type="text" name="EmailId" class="form-control" id="EmailId" placeholder="">
<div class="clearfix"></div>
</div>
</div>
...
function product_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "../ajax_files/ajax_branch.php",
   data:{action:action},  
...
function update_toast(){
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Store</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Store & You will not be able to recover this Product!",
            type: "warning",
            ...
            if (isConfirm) {
              ...
              product_lists();
                ...
                swal("Cancelled", "Store is safe :)", "error");
            }
        });
