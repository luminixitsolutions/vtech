...
if($searchValue != ''){
    ...
    if($Roll == 1 || $Roll == 7){
            $sql = "SELECT ts.*,tu.Fname FROM tbl_dealer_leads ts 
...
while ($row = mysqli_fetch_assoc($empRecords)) {
       ...
       $sql3 = "SELECT tpf.*,tu.Fname,tu.Lname FROM tbl_dealer_lead_details tpf LEFT JOIN tbl_users tu ON tpf.CreatedBy=tu.id WHERE tpf.CompId='".$row['id']."' AND tpf.CreatedDate='".date('Y-m-d')."' ORDER BY tpf.id DESC LIMIT 1";
                $rncnt3 = getRow($sql3);
                $row3 = getRecord($sql3);
                if($rncnt3 > 0){
    ...
    $data[] = array(
            ...
            "CellNo"=>$row['CellNo'],
            "Source"=>leadSourceDisplayHtml($row['ClainReason'], $row['Fname']),
