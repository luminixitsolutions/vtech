...
$DispatchOfficerId = itemTransferWorkflowResolveDispatchOfficerId(
    $conn,
    $Roll,
    $user_id,
    (int) ($_POST['DispatchOfficerId'] ?? 0)
);
...
if ($DispatchOfficerId <= 0) {
    echo "<script>alert('Please select a valid dispatcher.'); history.back();</script>";
    exit;
}
...
$ok = $ok && $conn->query("INSERT INTO tbl_dispatch_to_store_transfer SET DispatchOfficerId='$DispatchOfficerId', ToBranchId='$ToBranchId', TransferDate='$TransferDate', Narration='$Narration', CreatedBy='$user_id', CreatedDate='$CreatedDate'");
...
if (!empty($_POST['QtyProduct']) && is_array($_POST['QtyProduct'])) {
    foreach ($_POST['QtyProduct'] as $ProductId => $qty) {
        ...
        $avail = $conn->query("SELECT id, ProductName, Purity, Qty, ModelNo FROM tbl_distibute_item_details2
            WHERE StoreExeId='$DispatchOfficerId' AND ProductId='$ProductId' AND ProdType=0
...
if (!empty($_POST['SerialIds']) && is_array($_POST['SerialIds'])) {
    foreach ($_POST['SerialIds'] as $Detail2Id) {
        ...
        $dr = getRecord("SELECT id, ProductId, ProductName, Purity, Qty, SerialNo, ProdType, ModelNo FROM tbl_distibute_item_details2 WHERE id='$Detail2Id' AND StoreExeId='$DispatchOfficerId'");
        if (!$dr) {
            continue;
        }
