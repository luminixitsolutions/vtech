...
if (isset($_POST['submit'])) {
    $BagTitle   = addslashes(trim($_POST["Title"]));   
   ...
   if (!empty($_POST['ProductId'])) {
    foreach ($_POST['ProductId'] as $k => $prodId) {
        if (isset($_POST['checked'][$k])) { // checked index matches
            $prodName = addslashes($_POST['ProductName'][$k]);
            $qty      = (int) $_POST['Qty'][$k];
...
<input type="text" name="Title" class="form-control" id="Title" placeholder="" value='<?php echo $row7["Title"]; ?>' required>
...
<table id="example" class="table table-striped table-bordered" width="100%">
    ...
    <tr>
      ...
      <?php 
        ...
        foreach($row12 as $result){ ?>
          <tr>
  <td>
    <input type="checkbox" name="checked[<?php echo $i; ?>]" class="prod-check">
  </td>
  ...
  <td>
    <input type="number" name="Qty[<?php echo $i; ?>]" class="form-control" value="0" min="0">
  </td>
</tr>
