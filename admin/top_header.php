 ...
 * Include from any admin page after session + config.
 */

if (!function_exists('topHeaderAdminBasePath')) {
    function topHeaderAdminBasePath()
    {
...
$topHeaderBase = topHeaderAdminBasePath();
...
$topHeaderSearchAjax = $topHeaderBase . 'ajax-admin-menu-search.php';
...
if (!empty($_SESSION['Admin']['id'])) {
    ...
    $topNavRoll = (int) ($row77['Roll'] ?? $_SESSION['Admin']['Roll'] ?? 0);
    $topNavOptions = [];
    if (!empty($row77)) {
        $topNavOptions = adminResolveMenuOptionsFromUserRow($row77);
    }
    $topHeaderSearchIndexJson = json_encode(
        adminMenuSearchBuildIndex($topNavRoll, $topNavOptions, $topHeaderBase),
...
$topHeaderPhotoUrl = $topHeaderPhoto !== '' && $topHeaderSite !== ''
    ...
    : ($topHeaderSite !== '' ? $topHeaderSite . '/admin/user_icon.jpg' : $topHeaderBase . 'user_icon.jpg');
...
<nav class="layout-navbar navbar navbar-expand-lg align-items-lg-center bg-dark container-p-x top-header-bar" id="layout-navbar">
    ...
    <div class="navbar-collapse collapse" id="layout-navbar-collapse">
        <div class="top-header-search-wrap">
            <form id="topHeaderSearchForm" class="top-header-search-form" method="get" action="<?php echo htmlspecialchars($topHeaderSearchAction, ENT_QUOTES, 'UTF-8'); ?>" role="search" data-ajax="<?php echo htmlspecialchars($topHeaderSearchAjax, ENT_QUOTES, 'UTF-8'); ?>" data-base="<?php echo htmlspecialchars($topHeaderBase, ENT_QUOTES, 'UTF-8'); ?>">
