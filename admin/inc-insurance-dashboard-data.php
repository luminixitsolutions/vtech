...
function insuranceDashboardCountByProjectType($whereCondition, $projectType)
{
    global $conn;
    ...
    $sql = "SELECT COUNT(DISTINCT tdo.CustId) AS cnt
        ...
        WHERE $whereCondition AND tu.ProjectType = '$projectType'";
    $row = getRecord($sql);
    return (int) $row['cnt'];
}
...
function getInsuranceDashboardData()
{
    ...
    $siteDispatched = insuranceDashboardCount('tdo.Inst_Dispatcher_Otp_Verify = 1 AND tu.Roll = 5');
    $totalCompleted = insuranceDashboardCount(insuranceSiteCompletedSqlCondition());

    insuranceEnsureHistoryTable();
    $importedToday = (int) getRow("SELECT id FROM tbl_insurance_site_history WHERE CompletedDate = '$today'");
    $importedMonth = (int) getRow("SELECT id FROM tbl_insurance_site_history WHERE CompletedDate >= '$monthStart'");
    ...
    return array(
        ...
        'pump_pending' => insuranceDashboardCountByProjectType(insuranceSitePendingSqlCondition(), 1),
        'rooftop_pending' => insuranceDashboardCountByProjectType(insuranceSitePendingSqlCondition(), 2),
        'pump_active' => insuranceDashboardCountByProjectType(insuranceSiteActiveCompletedSqlCondition(), 1),
        'rooftop_active' => insuranceDashboardCountByProjectType(insuranceSiteActiveCompletedSqlCondition(), 2),
