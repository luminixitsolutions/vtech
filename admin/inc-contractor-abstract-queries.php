...
function contractorAbstractScopeSql($conn, $projectId, $subheadId, $dist = '', $alias = 'tu')
{
    ...
    $sql = " AND $alias.ProjectType=1 AND $alias.ProjectId='$projectId' AND $alias.ProjectSubHeadId='$subheadId' AND $alias.Roll=5";
...
function contractorAbstractListSql($conn, $metric, $projectId, $subheadId, $dist = '', $contractorId = 0)
{
    ...
    if ($metric === 'material_dispatch') {
        return "SELECT tu.* FROM tbl_users tu
            INNER JOIN tbl_sell ts ON ts.CustId=tu.id AND ts.Inst_Dispatcher_Otp_Verify=1
    ...
    if ($metric === 'today_installation') {
        return "SELECT tu.* FROM tbl_users tu
            INNER JOIN tbl_installations ti ON ti.CustId=tu.id AND ti.InstallStatus='Yes' AND ti.Type=2
...
function contractorAbstractDistricts($conn, $projectId, $subheadId, $districtFilter = '')
{
    ...
    $sql = "SELECT DISTINCT tu.District FROM tbl_users tu
        WHERE tu.District!='' AND tu.ProjectType=1 AND tu.ProjectId='$projectId'
...
function contractorAbstractTotalSites($conn, $projectId, $subheadId, $districtFilter = '')
{
    $projectId = (int) $projectId;
    $subheadId = (int) $subheadId;
    $sql = "SELECT COUNT(*) AS cnt FROM tbl_users tu WHERE tu.Roll=5 AND tu.ProjectType=1 AND tu.ProjectId='$projectId' AND tu.ProjectSubHeadId='$subheadId' AND tu.District!=''";
