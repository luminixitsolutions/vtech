...
$bhScopeSql = installationWorkflowBhExtensionScopeSql($user_id);
$gmExtFilter = installationWorkflowBhExtensionFilterSql('e');
...
$customerOptions = installationWorkflowExtensionCustomerOptions($bhScopeSql, $gmExtFilter, 'mgr');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'approve') {
        $result = installationWorkflowApproveExtension($extId, $user_id, $bhScopeSql);
        echo !empty($result['ok']) ? 'OK' : 'ERR';
        exit;
    }

    if ($action === 'reject') {
        $result = installationWorkflowRejectExtension($extId, $user_id, $bhScopeSql);
        echo !empty($result['ok']) ? 'OK' : 'ERR';
        exit;
    }
}
...
<head>
    <title><?php echo $Proj_Title; ?> | Business Head Extension Requests</title>
    <meta charset="utf-8">
...
<h4 class="font-weight-bold mb-3">Business Head - Extension Requests</h4>
...
<form method="get" class="form-row align-items-end" id="filterForm">
    ...
    <div class="form-group col-md-2 mb-0">
        <a href="bh-extension-requests.php" class="btn btn-secondary btn-block">Reset</a>
    </div>
</form>
</div></div>
...
<thead><tr>
    <th>#</th><th>Beneficiary ID</th><th>Customer Name</th><th>Requested By (GM)</th><th>Days</th><th>Requested Date</th><th>Status</th><th>Remarks</th><th>Action</th>
</tr></thead>
<tbody>
<?php
$i = 1;
...
function reloadAfterAction(newStatus){
    ...
    window.location.href = 'bh-extension-requests.php?status=' + encodeURIComponent(newStatus) + '&customer_id=' + encodeURIComponent(customerId);
}

function approve(id){
    if(!confirm('Approve extension?')) return;
...
function reject(id){
    if(!confirm('Reject extension?')) return;
