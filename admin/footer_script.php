    ...
    <?php
    $__admin_script = isset($_SERVER['SCRIPT_NAME']) ? str_replace('\\', '/', (string) $_SERVER['SCRIPT_NAME']) : '';
    $__top_header_js = (strpos($__admin_script, '/admin/') !== false) ? 'js/top-header-bar.js' : '';
    if ($__top_header_js !== '' && is_file(__DIR__ . '/' . $__top_header_js)) {
        $__th_depth = 0;
        if (preg_match('#/admin/(.+)$#i', $__admin_script, $__th_m)) {
    ...
    <script>
    (function ($) {
        if ($.growl && typeof $.growl.success !== 'function' && typeof $.growl.notice === 'function') {
            $.growl.success = function (opts) {
                return $.growl.notice(opts);
            };
        }
    })(jQuery);
    </script>
