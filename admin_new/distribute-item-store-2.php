    ...
    <table id="example3" class="table table-striped table-bordered" width="100%">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <label class="custom-control custom-checkbox">
              <input type="checkbox" id="Check_Id<?php echo $result['id']; ?>" 
                     value="0" 
                     class="custom-control-input is-valid checkbox2" 
          ...
          <input type="hidden" value="0" class="chkvalue2" name="CheckId2[]" id="CheckId2<?php echo $result['id']; ?>">      
...
function setupSelectAll(selectAllId, checkboxClass, hiddenPrefix) {
    const selectAllCheckbox = document.getElementById(selectAllId);
    const checkboxes = document.querySelectorAll(`.${checkboxClass}`);

    if (!selectAllCheckbox) return; // exit if element not found
    ...
    selectAllCheckbox.addEventListener('change', function () {
        ...
        checkboxes.forEach(checkbox => {
            ...
            // Extract numeric ID (e.g., 25 from Check_Id25)
            const idPart = checkbox.id.replace('Check_Id', '');

            // Try both possible hidden field prefixes
            const hiddenInput1 = document.getElementById('CheckId' + idPart);
            const hiddenInput2 = document.getElementById('CheckId2' + idPart);

            // Update whichever exists
            if (hiddenInput1) hiddenInput1.value = isChecked ? 1 : 0;
            if (hiddenInput2) hiddenInput2.value = isChecked ? 1 : 0;
        });
    });
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            const idPart = this.id.replace('Check_Id', '');

            const hiddenInput1 = document.getElementById('CheckId' + idPart);
            const hiddenInput2 = document.getElementById('CheckId2' + idPart);

            if (hiddenInput1) hiddenInput1.value = this.checked ? 1 : 0;
            if (hiddenInput2) hiddenInput2.value = this.checked ? 1 : 0;
...
setupSelectAll('selectAll', 'checkbox', 'CheckId');
setupSelectAll('selectAll2', 'checkbox2', 'CheckId2');
