...
$id = $_GET['id'];
$id = $_GET['id'];
$sql7 = "SELECT tp.*,tu.GstNo,tu.EmailId,tu.Phone2,tu2.Fname As CompName,tu2.Address As CompAddress,tu2.EmailId AS CompEmailId,tu2.Phone As CompPhone,tu2.Phone2 AS CompPhone2,tu3.Fname As AgencyName,tu3.Phone As AgencyPhone,tu3.EmailId As AgencyEmailId,tu2.Photo AS CompLogo,tu3.Address As AgencyAddress,tcm.Name AS Pump_Capacity FROM tbl_sell tp 
...
$sql2 = "SELECT SUM(Qty) AS TotQty FROM `tbl_sell_products` WHERE SellId='$id' AND ProdType!=2";
$row2 = getRecord($sql2);
$TotQty = $row2['TotQty'];
...
<p><strong>Capacity:</strong> <?php echo $row['Pump_Capacity'];?></p>
...
<tr style="background-color:#b3ffff;">
  ...
  <td colspan="2" width="332"><strong>P R O D U C T</strong></td>
  <td colspan="2" width="145"><strong>MODEL NO.</strong></td>
  <td width="155"><strong>SERIAL NO.</strong></td>
...
// 🔹 Step 1: Fetch all bag (main) products first (ProdType=2)
$sql_bags = "SELECT * FROM tbl_sell_products WHERE SellId='$id' AND ProdType='2' ORDER BY id ASC";
$bags = getList($sql_bags);

if ($bags) {
  foreach ($bags as $bag) {
    $bagId     = $bag['ProductId'];
    $bagName   = $bag['ProductName'];
    $bagModel  = $bag['ModelNo'];
    $bagSerial = $bag['SerialNo'];
    $bagQty    = $bag['Qty'];
?>

<!-- 👜 BAG HEADER ROW -->
<tr style="background: linear-gradient(90deg, #b3ffff, #ccf5ff); font-weight:bold;">
  <td><?php echo $i++; ?></td>
  <td colspan="4">
    👜 <strong><?php echo strtoupper($bagName); ?></strong>
  </td>
  <td><?php echo $bagSerial; ?></td>
  <td><?php echo $bagQty; ?></td>
</tr>

<?php
    // 🔹 Step 2: Fetch sub-items (ProdType=0) linked to this bag
    $sql_sub = "SELECT * FROM tbl_sell_products 
                WHERE SellId='$id' AND ProdType='0' AND BagId='$bagId'
                ORDER BY id ASC";
    $subItems = getList($sql_sub);
...
<tr>
  <td></td>
  <td colspan="2" style="padding-left:20px;">- <?php echo $sub['ProductName']; ?></td>
  <td colspan="2"><?php echo $sub['ModelNo']; ?></td>
  <td><?php echo $sub['SerialNo']; ?></td>
...
<tr>
  <td></td>
  <td colspan="6" style="color:#888; font-style:italic; padding-left:30px;">No sub-items linked</td>
</tr>
<?php
    }
  }
...
// 🔹 Step 3: Fetch remaining grouped products (non-bag items)
$sql_others = "SELECT * FROM tbl_sell_products 
               WHERE SellId='$id' AND (ProdType IS NULL OR ProdType NOT IN (0,2))
               ORDER BY ProductName, id ASC";
$otherItems = getList($sql_others);

if ($otherItems) {
  $grouped = [];
  foreach ($otherItems as $prod) {
    $grouped[$prod['ProductName']][] = $prod;
  }

  foreach ($grouped as $productName => $items) {
?>
<tr style="background-color:#e6faff; font-weight:bold;">
  <td><?php echo $i++; ?></td>
  <td colspan="4"><strong><?php echo $productName; ?></strong></td>
  <td></td>
  <td></td>
</tr>
...
<tr>
  <td></td>
  <td colspan="2" style="padding-left:25px;">-<?php echo $sub['SerialNo']; ?></td>
  <td colspan="2"><?php echo $sub['ModelNo']; ?></td>
  <td><?php echo $sub['SerialNo']; ?></td>
  <td><?php echo $sub['Qty']; ?></td>
</tr>
