...
$MainPage = "Bag-Purchase-Order";
$Page = "View-Bag-Purchase-Order";
...
if(isset($_POST['submit'])){
    ...
    $conn->query($sql);
    echo "<script>alert('Order Applied Successfully');window.location.href='take-bag-po-action.php?id=$id';</script>";
}
...
if(isset($_POST['submit2'])){
    ...
    $conn->query($sql);
    echo "<script>alert('Order Send To Manufacture Successfully');window.location.href='take-bag-po-action.php?id=$id';</script>";
}
...
if(isset($_POST['submit3'])){
    ...
    $conn->query($sql);
    echo "<script>alert('Order Receive From Manufacture Successfully');window.location.href='take-bag-po-action.php?id=$id';</script>";
}
...
if(isset($_POST['submit4'])){
    ...
    $sql = "UPDATE tbl_purchase_order SET DeliveredDate='$DeliveredDate',DeliveredTime='$DeliveredTime',DeliveredStatus=1,BranchId='$BranchId',BillNo='$BillNo' WHERE id='$id'";
    $conn->query($sql);
  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        $sheetCount = count($Reader->sheets());
         $sql = "DELETE FROM tbl_stocks WHERE SellId='$id' AND ProdType='2' AND SellType='Purchase'";
$conn->query($sql);
...
$OemVedName = "";
                ...
                $conn->query($sql22);
                    ...
                    foreach($row as $result){
                        ...
                        $Qty2 = $result['Qty'];
                        $sql22 = "INSERT INTO tbl_stocks SET VehicalDate='$VehicalDate',VehicalNo='$VehicalNo',CustomerId='$CustomerId',CompId='$CompId',SellId='$id',ProductId='$prodid',ProductName='$Product_Name',Qty='$Qty2',Status='1',CrDr='cr',CreatedBy='$user_id',CreatedDate='$DeliveredDate',Narration='Stock Added',PostId='$PostId',BranchId='$BranchId',SellType='Purchase',SerialNo='$SerialNo',ModelNo='$ModelNo',SrNo='$SrNo',ProdType='0',BillNo='$BillNo'";
                $conn->query($sql22);
                    }
 ...
 $sql = "DELETE FROM tbl_stocks WHERE SrNo='SrNo' AND ProdType IN(0,2)";
$conn->query($sql);
?>
<script>
alert("Excel Data Imported into the Database");
    window.location.href='take-bag-po-action.php?id=<?php echo $id;?>';
</script>
<?php
  }
  else
...
$OemVedName = "";
                if(isset($Row[11])) {
                 ...
                 if (!empty($SrNo) || !empty($ProductId) || !empty($ProductName) || !empty($SerialNo) || !empty($Qty) || !empty($ModelNo) || !empty($CompId) || !empty($PostId)) {
                      ...
                      $sql22 = "INSERT INTO tbl_stocks SET VehicalDate='$VehicalDate',VehicalNo='$VehicalNo',CustomerId='$CustomerId',CompId='$CompId',SellId='$id',ProductId='$ProductId',ProductName='$ProductName',Qty='$Qty',Status='1',CrDr='cr',CreatedBy='$user_id',CreatedDate='$DeliveredDate',Narration='Stock Added',PostId='$PostId',BranchId='$BranchId',SellType='Purchase',SerialNo='$SerialNo',ModelNo='$ModelNo',SrNo='$SrNo',ProdType='1',Unit='$Unit',BillNo='$BillNo',OemVedName='$OemVedName'";
                $conn->query($sql22);
...
<table id="example2" class="table table-striped table-bordered nowrap" style="width:100%">
        ...
        <tbody>
            <?php 
            $i3=1;
            $sql = "SELECT * FROM tbl_stocks WHERE SellId='".$_GET['id']."' AND ProdType='2' AND SellType='Purchase' ORDER BY ProductId ASC";
            $rncnt = getRow($sql);
            if($rncnt > 0){
            $sql66 = "SELECT tpo.* FROM tbl_stocks tpo WHERE tpo.SellId='".$_GET['id']."' AND ProdType='2' AND SellType='Purchase'";
            ...
            <tr>
                ...
                <td><a href="javascript:void(0)" 
                   class="view-items" 
                   data-id="<?php echo $result['ProductId']; ?>" 
                   data-title="<?php echo htmlspecialchars($result['ProductName']); ?>"><?php echo $result['ProductName'];?></a></td>
        ...
        <?php 
                ...
                $sql66 = "SELECT tpo.*,tp.ModelNo FROM tbl_purchase_order_products tpo INNER JOIN tbl_products tp ON tpo.ProductId=tp.id WHERE tpo.SellId='".$_GET['id']."' AND tp.Roll=2";
                 $row66 = getList($sql66);
            foreach($row66 as $result){
               for($i=1;$i<=$result['Qty'];$i++){

                    $sql = "SELECT SerialNo FROM tbl_stocks WHERE SellId='".$_GET['id']."' AND ProductId='".$result['ProductId']."' AND SrNo='$i' AND SellType='Purchase'";
                    $row = getRecord($sql);
...
<script type="text/javascript">
    ...
    $(document).on('click', '.view-items', function() {
        ...
        $.ajax({
            url: 'product_management/get_bag_items.php',
            type: 'GET',
            data: { bag_id: bagId },
