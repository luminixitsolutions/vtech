...
$query = "SELECT * FROM tbl_users WHERE (Phone = '$Username' OR EmailId='$Username') AND Status=1 AND Roll IN(34,35,36,37)";
 $rncnt = getRow($query);
 $row = getRecord($query);
 if($rncnt > 0){
...
if($_POST['action']=='DispatchOtpVerify'){
$getotp = addslashes(trim($_POST['getotp']));
$YourOtp = addslashes(trim($_POST['YourOtp']));
$id = addslashes(trim($_POST['id']));
if($getotp == $YourOtp){
    $sql = "UPDATE tbl_sell SET Inst_Dispatcher_Otp_Verify=1 WHERE id='$id'";
    $conn->query($sql);
    unset($_SESSION['otp']);   
    echo 1;
}
...
if($_POST['action']=='InstallationOtpVerify'){
$getotp = addslashes(trim($_POST['getotp']));
$YourOtp = addslashes(trim($_POST['YourOtp']));
$id = addslashes(trim($_POST['id']));
$InstId = addslashes(trim($_POST['InstId']));
if($getotp == $YourOtp){
    $sql2 = "SELECT InstallationDate FROM tbl_users WHERE id='$id'";
    $row2 = getRecord($sql2);
    $InstallationDate = $row2['InstallationDate'];
    $sql = "UPDATE tbl_users SET InstOtpVerify=1 WHERE id='$id'";
    $conn->query($sql);
    
    $sql = "UPDATE tbl_installations SET InstallStatus='Yes',InstallationDate='$InstallationDate' WHERE CustId='$id' AND id='$InstId'";
    $conn->query($sql);
...
if($_POST['action']=='InspectionOtpVerify'){
$getotp = addslashes(trim($_POST['getotp']));
$YourOtp = addslashes(trim($_POST['YourOtp']));
$id = addslashes(trim($_POST['id']));
if($getotp == $YourOtp){
    $sql2 = "SELECT InstInspectionDate FROM tbl_users WHERE id='$id'";
    $row2 = getRecord($sql2);
    $InstInspectionDate = $row2['InstInspectionDate'];
    
    $sql = "UPDATE tbl_users SET InspectionOtpVerify=1 WHERE id='$id'";
    $conn->query($sql);
    
    $sql = "UPDATE tbl_installations SET PoInspection='Yes',PoInspectionDate='$InstInspectionDate' WHERE CustId='$id'";
    $conn->query($sql);
...
if($_POST['action']=='RooftopDispatchOtpVerify'){
  $getotp = addslashes(trim($_POST['getotp']));
  $YourOtp = addslashes(trim($_POST['YourOtp']));
  $id = addslashes(trim($_POST['id']));
  if($getotp == $YourOtp){
      $sql = "UPDATE tbl_rooftop_sell SET Inst_Dispatcher_Otp_Verify=1 WHERE id='$id'";
      $conn->query($sql);
      unset($_SESSION['otp']);   
      echo 1;
  }
