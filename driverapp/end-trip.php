...
if (isset($_POST['submit'])) {
    ...
    $conn->query($sql);
    appFlashSet('success', 'Trip ended successfully!');
...
$row7 = getRecord($sql);
$userRow = getRecord("SELECT Lattitude, Longitude FROM tbl_users WHERE id='$user_id' LIMIT 1") ?: [];
$Latitude = trim($userRow['Lattitude'] ?? '');
$Longitude = trim($userRow['Longitude'] ?? '');
if (!empty($row7['EndLattitude'])) {
    $Latitude = trim($row7['EndLattitude']);
    $Longitude = trim($row7['EndLongitude'] ?? '');
}
?>
<!DOCTYPE html>
