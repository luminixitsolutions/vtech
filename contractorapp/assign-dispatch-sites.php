...
?>
              ...
              <?php } ?>
                                        ...
                                        <a href="assign-dispatch-sites.php" class="btn btn-info">Clear</a>
                                        </div>
                                        </div>
                                        </form>
                                         </div>
  ...
  <div class="card-header" style="padding: 10px;">
            ...
            {
                $sql2 = "SELECT * FROM tbl_sell WHERE DispatcherStatus=1 AND id='".$row['id']."'";
                    $rncnt2 = getRow($sql2);
                    if($rncnt2 > 0){}
                        else{
...
$InstallerName = $row['Fname'];
    ...
    /*$sql = "INSERT INTO tbl_allocate_sites_to_installer SET CustId='$item',InstallerId='$InstallerId',CreatedDate='$CreatedDate',CreatedBy='$user_id',AssignFor=2";
    $conn->query($sql);*/
    $sql = "UPDATE tbl_sell SET DispatcherStatus='1',DispatcherId='$InstallerId',DispatcherDate='$CreatedDate',ContractorId='$user_id' WHERE id='$item'";
    $conn->query($sql);
    
    $sql2 = "SELECT tu.id,tu.Fname,tu.Phone AS CustPhone,tc.Name AS PumpCapacity,tu2.Fname AS DriverName,tu2.VehicalNo,tu2.Phone AS DriverPhone FROM `tbl_users` tu INNER JOIN tbl_common_master tc ON tc.id=tu.PumpCapacity INNER JOIN tbl_sell ts ON ts.CustId=tu.id INNER JOIN tbl_users tu2 ON ts.DriverId=tu2.id WHERE tu.id='$item'";
$row2 = getRecord($sql2);
$CustName = $row2['Fname'];
...
$response = sendWhatsAppTemplate($phone, $template, $params,$languageCode);
    ...
    echo "<script>window.location.href='assign-dispatch-sites.php';</script>";
        }
   ?>             
<form method="post">
