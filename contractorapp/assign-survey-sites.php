...
?>
              ...
              <?php } ?>
                                        ...
                                        <a href="assign-survey-sites.php" class="btn btn-info">Clear</a>
                                        </div>
                                        </div>
                                        </form>
                                         </div>
  ...
  <div class="card-header" style="padding: 10px;">
            ...
            $sql = "SELECT * FROM tbl_users WHERE FieldSurveyId='$user_id' AND Roll IN(5)";
            if($_REQUEST['SearchCustomer']!=''){
                $searchTerm = $conn->real_escape_string($_REQUEST['SearchCustomer']);
                $sql.=" AND (Fname LIKE '%".$searchTerm."%' OR Lname LIKE '%".$searchTerm."%' OR BeneficiaryId LIKE '%".$searchTerm."%' OR CONCAT(Fname,' ',Lname) LIKE '%".$searchTerm."%')";
            }
            if($_REQUEST['Village']!=''){
                $sql.=" AND Village='".$_REQUEST['Village']."'";
            }
            if($_REQUEST['Taluka']!=''){
                $sql.=" AND Taluka='".$_REQUEST['Taluka']."'";
            }
            if($_REQUEST['District']!=''){
                $sql.=" AND District='".$_REQUEST['District']."'";
            }
            ...
            {
                    $sql2 = "SELECT * FROM tbl_allocate_sites_to_installer WHERE CustId='".$row['id']."' AND AssignFor=1";
                    $rncnt2 = getRow($sql2);
                    if($rncnt2 > 0){}
                        else{
             ?>
<div class="card mb-4" id="bgcolor<?php echo $row['id'];?>">
   ...
   $CreatedDate = date('Y-m-d H:i:s');
    ...
    $sql = "INSERT INTO tbl_allocate_sites_to_installer SET CustId='$item',InstallerId='$InstallerId',CreatedDate='$CreatedDate',CreatedBy='$user_id',AssignFor=1";
    $conn->query($sql);
    }

    
    echo "<script>window.location.href='assign-survey-sites.php';</script>";
        }
   ?>             
<form method="post">
...
<option selected="" value="">Select</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll IN(34,35,36,37) AND UnderInstallerId='$user_id' AND Options LIKE '%85%'";
  $row12 = getList($sql12);
...
<button type="submit" name="submit" class="btn btn-primary btn-finish">Submit</button>
            ...
            while($row = $res->fetch_assoc())
            {
                    ...
                    if($rncnt2 > 0){
                        $row2 = getRecord($sql2);
                        $sql3 = "SELECT Fname FROM tbl_users WHERE id='".$row2['InstallerId']."'";
                        $row3 = getRecord($sql3);
             ?>
<div class="card mb-4" id="bgcolor<?php echo $row['id'];?>">
