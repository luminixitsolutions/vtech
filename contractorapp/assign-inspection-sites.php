...
<div class="container">
  <h4 class="font-weight-bold py-3 mb-0">Inspection Sites</h4>
    <div class="card mb-4">
...
?>
              ...
              <?php } ?>
                                        ...
                                        <a href="assign-inspection-sites.php" class="btn btn-info">Clear</a>
                                        </div>
                                        </div>
                                        </form>
                                         </div>
  ...
  <div class="card-header" style="padding: 10px;">
           ...
           /* $sql = "SELECT tdo.*,tu.id AS Cust_Id,tu.Fname,tu.EmailId,tu.Phone,tu.Address,tu.ProjectType,tu.BeneficiaryId FROM tbl_sell tdo 
            ...
            $sql = "SELECT tdo.*,tu.id AS Cust_Id,tu.Fname,tu.Lname,tu.EmailId,tu.Phone,tu.Address,tu.ProjectType,tu.BeneficiaryId FROM tbl_sell tdo 
                    LEFT JOIN tbl_users tu ON tdo.CustId=tu.id WHERE tu.ContractorInspectionId='$user_id'";
            ...
            {
                $sql2 = "SELECT * FROM tbl_users WHERE InspectionStatus=1 AND id='".$row['Cust_Id']."'";
                    $rncnt2 = getRow($sql2);
                    if($rncnt2 > 0){}
                        else{
...
$InstallerName = $row['Fname'];
   ...
   $CreatedDate = date('Y-m-d H:i:s');
    foreach ($array as $item) {
         $sql = "UPDATE tbl_users SET InspectionStatus='1',InspectionId='$InstallerId',AssignInspectionDate='$CreatedDate',InspectionContractorId='$user_id' WHERE id='$item'";
                $conn->query($sql);
...
$Location = $row2['Address'];
// customer 
$phone = "91".$CustPhone;
$template = "inspection";
$params = [$CustName, $DispatchDate, $Location];  // Name, Date, Location
...
$response = sendWhatsAppTemplate($phone, $template, $params,$languageCode);
    ...
    echo "<script>window.location.href='assign-inspection-sites.php';</script>";
        }
   ?>             
<form method="post">
...
<label class="form-label"> Inspectionar<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="InstallerId" id="InstallerId" required>
<option selected="" value="">Select</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll IN(34,35,36,37) AND UnderInstallerId='$user_id' AND Options LIKE '%88%'";
  $row12 = getList($sql12);
...
<button type="submit" name="submit" class="btn btn-primary btn-finish">Submit</button>
           ...
           /* $sql = "SELECT tdo.*,tu.id AS Cust_Id,tu.Fname,tu.EmailId,tu.Phone,tu.Address,tu.ProjectType,tu.BeneficiaryId FROM tbl_sell tdo 
            ...
            while($row = $res->fetch_assoc())
            {
                     $sql2 = "SELECT tu2.Fname As CoName,tu.InspectionDate FROM tbl_users tu 
                          LEFT JOIN tbl_users tu2 ON tu2.id=tu.InspectionId 
                          WHERE tu.InspectionStatus=1 AND tu.id='".$row['Cust_Id']."'";
                    $rncnt2 = getRow($sql2);
                    if($rncnt2 > 0){
                        $row2 = getRecord($sql2);
