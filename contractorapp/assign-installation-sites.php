...
<div class="container">
  <h4 class="font-weight-bold py-3 mb-0">Installation Sites</h4>
     <div class="card mb-4">

                    <div class="card-body">
      <form id="search-form" method="get" action="">
                                                <div class="form-row">
    <div class="form-group col-md-6 col-12" style="margin-bottom: 15px;">
                                            <label class="form-label">Search Customer</label>
                                            <input type="text" class="form-control" id="SearchCustomer" name="SearchCustomer" placeholder="Enter Customer Name or ID..." value="<?php echo isset($_REQUEST['SearchCustomer']) ? htmlspecialchars($_REQUEST['SearchCustomer']) : ''; ?>">
                                            <small class="text-muted">Search by Name or Beneficiary ID</small>
...
?>
              ...
              <?php } ?>
                                        ...
                                        <a href="assign-installation-sites.php" class="btn btn-info">Clear</a>
                                        </div>
                                        </div>
                                        </form>
                                         </div>
  ...
  <div class="card-header" style="padding: 10px;">
             ...
             $sql = "SELECT tdo.*,tu.id AS Cust_Id,tu.Fname,tu.Lname,tu.EmailId,tu.Phone,tu.Address,tu.ProjectType,tu.BeneficiaryId FROM tbl_sell tdo 
                    LEFT JOIN tbl_users tu ON tdo.CustId=tu.id WHERE tu.ContractorInstallerId='$user_id'";
            if($_REQUEST['SearchCustomer']!=''){
                $searchTerm = $conn->real_escape_string($_REQUEST['SearchCustomer']);
                $sql.=" AND (tu.Fname LIKE '%".$searchTerm."%' OR tu.Lname LIKE '%".$searchTerm."%' OR tu.BeneficiaryId LIKE '%".$searchTerm."%' OR CONCAT(tu.Fname,' ',tu.Lname) LIKE '%".$searchTerm."%')";
            }        
            if($_REQUEST['Village']!=''){
            ...
            {
                $sql2 = "SELECT * FROM tbl_users WHERE InstallerStatus=1 AND id='".$row['Cust_Id']."'";
                    $rncnt2 = getRow($sql2);
                    if($rncnt2 > 0){}
                        else{
...
<div class="card mb-4" id="bgcolor<?php echo $row['Cust_Id'];?>">
                    ...
                    <div class="card-body">
                        <h6 style="margin-bottom: 1px;font-size: 15px;"><input type="checkbox" id="Check_Id<?php echo $row['Cust_Id']; ?>" value="<?php echo $row['Cust_Id']; ?>" onclick="featured(<?php echo $row['Cust_Id']; ?>)" class="common_selector Proccedures">&nbsp;&nbsp;&nbsp;<?php echo $row['Fname']; ?></h6>
                        <input type="hidden" value="0" name="CheckId[]" id="CheckId<?php echo $row['Cust_Id']; ?>">
             ...
             $sql = "SELECT tdo.*,tu.id AS Cust_Id,tu.Fname,tu.Lname,tu.EmailId,tu.Phone,tu.Address,tu.ProjectType,tu.BeneficiaryId FROM tbl_rooftop_sell tdo 
                    LEFT JOIN tbl_users tu ON tdo.CustId=tu.id WHERE tu.ContractorInstallerId='$user_id'";
            if($_REQUEST['SearchCustomer']!=''){
                $searchTerm = $conn->real_escape_string($_REQUEST['SearchCustomer']);
                $sql.=" AND (tu.Fname LIKE '%".$searchTerm."%' OR tu.Lname LIKE '%".$searchTerm."%' OR tu.BeneficiaryId LIKE '%".$searchTerm."%' OR CONCAT(tu.Fname,' ',tu.Lname) LIKE '%".$searchTerm."%')";
...
$InstallerName = $row['Fname'];
    ...
    $sql = "UPDATE tbl_users SET InstallerStatus='1',InstallerId='$InstallerId',InstallerDate='$CreatedDate',InstallerContractorId='$user_id' WHERE id='$item'";
    $conn->query($sql);
    
     $sql2 = "SELECT tu.id,tu.Fname,tu.Address,tu.Phone AS CustPhone,tc.Name AS PumpCapacity FROM `tbl_users` tu INNER JOIN tbl_common_master tc ON tc.id=tu.PumpCapacity 
     WHERE tu.id='$item'";
$row2 = getRecord($sql2);
$CustName = $row2['Fname'];
...
// customer 
$phone = "91".$CustPhone;
$template = "installsationinformation";
$params = [$CustName, $DispatchDate, $Location, $InstallerName, $InstallerPhone];  // Name, Date, Location
...
//uinstaller 
$phone = "91".$InstallerPhone;
$template = "installationassignment";
$params = [$InstallerName,$CustName, $CustAddress, $PumpCapacity, $CustPhone];  // Name, Date, Location
...
$response = sendWhatsAppTemplate($phone, $template, $params,$languageCode);
    ...
    echo "<script>window.location.href='assign-installation-sites.php';</script>";
        }
   ?>             
<form method="post">
...
<label class="form-label"> Installer<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="InstallerId" id="InstallerId" required>
<option selected="" value="">Select</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll IN(34,35,36,37) AND UnderInstallerId='$user_id' AND Options LIKE '%87%'";
  $row12 = getList($sql12);
...
<button type="submit" name="submit" class="btn btn-primary btn-finish">Submit</button>
           ...
           /* $sql = "SELECT tdo.*,tu.id AS Cust_Id,tu.Fname,tu.EmailId,tu.Phone,tu.Address,tu.ProjectType,tu.BeneficiaryId FROM tbl_sell tdo 
            ...
            while($row = $res->fetch_assoc())
            {
                    $sql2 = "SELECT tu2.Fname As CoName,tu.InstallerDate FROM tbl_users tu 
                          LEFT JOIN tbl_users tu2 ON tu2.id=tu.InstallerId 
                          WHERE tu.InstallerStatus=1 AND tu.id='".$row['CustId']."'";
                    $rncnt2 = getRow($sql2);
                    if($rncnt2 > 0){
                        $row2 = getRecord($sql2);
...
<div class="card mb-4" id="bgcolor<?php echo $row['Cust_Id'];?>">
                  ...
                  <?php } ?>
                   ...
                   <p style="margin-bottom: 1px;"><strong>Assign To :</strong> <?php echo $row2['CoName']; ?> </p>
